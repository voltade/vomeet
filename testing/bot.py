...
class Bot:
    ...
    def __init__(
        self, user_client: VomeetClient, meeting_url: str, bot_id: Optional[str] = None
    ):
        """
    ...
    def get_transcript(self) -> Dict[str, Any]:
        ...
        if not self.created:
            raise Exception(
                f"Bot {self.bot_id} has not been created yet. Call create() first."
            )

        def _get_transcript():
            return self.user_client.get_transcript(
                platform=self.platform, native_meeting_id=self.native_meeting_id
            )
        ...
        try:
            ...
            if segments:
                ...
                for seg in segments:
                    ...
                    if abs_start:
                        if first_abs_start is None or pd.to_datetime(
                            abs_start
                        ) < pd.to_datetime(first_abs_start):
                            first_abs_start = abs_start
                    if abs_end:
                        if last_abs_end is None or pd.to_datetime(
                            abs_end
                        ) > pd.to_datetime(last_abs_end):
    ...
    def update_config(
        self, language: Optional[str] = None, task: Optional[str] = None
    ) -> Dict[str, Any]:
        """
