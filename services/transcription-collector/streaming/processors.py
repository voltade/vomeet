...
from shared_models.schemas import (
    Platform,
)  # WhisperLiveData not directly used by these functions from snippet
from config import (
    REDIS_SEGMENT_TTL,
    REDIS_SPEAKER_EVENT_KEY_PREFIX,
    REDIS_SPEAKER_EVENT_TTL,
)  # Added new configs (NEW)
...
from mapping.speaker_mapper import (
    ...
    STATUS_UNKNOWN,
    STATUS_ERROR,
)  # Removed direct map_speaker_to_segment and other statuses if not directly used by this file
...
def verify_meeting_token(token: str) -> Optional[dict]:
    try:
        ...
        if "exp" in payload and int(payload["exp"]) < now:
            return None
        if (
            payload.get("aud") != "transcription-collector"
            or payload.get("iss") != "bot-manager"
        ):
            return None
...
async def process_session_start_event(
    message_id: str,
    ...
    try:
        ...
        if not all(field in stream_data for field in required_fields):
            logger.warning(
                f"Session start message {message_id} missing required fields for session processing. Skipping. Required: {required_fields}"
            )
        ...
        try:
            ...
            logger.warning(
                f"Invalid timestamp format in session_start message {message_id}: {e}. Data: {start_timestamp_str}"
            )
        ...
        if meeting_session:
            ...
            logger.info(
                f"Updated start time for existing session {session_uid}, meeting_id {meeting.id} to {start_timestamp}"
            )
        else:
            ...
            logger.info(
                f"Created new session {session_uid} for meeting_id {meeting.id} with start time {start_timestamp}"
            )
        ...
        try:
            ...
            await redis_c.set(
                session_start_cache_key, start_timestamp.isoformat(), ex=7200
            )  # 2 hour TTL
            logger.info(
                f"Cached session start time in Redis: {session_start_cache_key}"
            )
        except Exception as redis_err:
            logger.warning(
                f"Failed to cache session start time in Redis for session {session_uid}: {redis_err}"
            )

        logger.info(
            f"Successfully processed session_start event for meeting {meeting.id}, session {session_uid}"
        )
        return True

    except Exception as e:
        logger.error(
            f"Error processing session_start_event for message {message_id}, meeting {meeting.id if meeting else 'Unknown'}: {e}",
            exc_info=True,
        )
        try:
            ...
            logger.error(
                f"Failed to rollback after error in process_session_start_event: {rb_err}",
                exc_info=True,
            )
...
async def process_stream_message(
    message_id: str, message_data: Dict[str, Any], redis_c: aioredis.Redis
) -> bool:
    ...
    try:
        ...
        async with async_session_local() as db:
            try:
                ...
                if not claims:
                    logger.warning(
                        f"Message {message_id} (type: {message_type}) failed MeetingToken verification. Skipping."
                    )
                    return True

                internal_meeting_id = int(claims.get("meeting_id"))
                platform_val = claims.get("platform") or stream_data.get("platform")
                native_meeting_id = claims.get("native_meeting_id") or stream_data.get(
                    "meeting_id"
                )
                ...
                if message_type == "session_start":
                    ...
                    if not meeting:
                        logger.warning(
                            f"Session start for unknown meeting_id {internal_meeting_id}. Skipping."
                        )
                        return True
                    return await process_session_start_event(
                        message_id, stream_data, db, None, meeting, redis_c
                    )
                elif message_type == "transcription":
                    pass  # Continue with transcription processing
                elif (
                    message_type == "session_end"
                ):  # NEW: Handle session_end for cleanup
                    session_uid = stream_data.get("uid")
                    if not session_uid:
                        logger.warning(
                            f"Message {message_id} (type: session_end) missing 'uid'. Skipping cleanup."
                        )
                        return True  # Cannot process without UID, but ack

                    speaker_event_key = (
                        f"{REDIS_SPEAKER_EVENT_KEY_PREFIX}:{session_uid}"
                    )
                    session_start_cache_key = f"meeting_session:{session_uid}:start"
                    try:
                        deleted_count = await redis_c.delete(
                            speaker_event_key, session_start_cache_key
                        )
                        logger.info(
                            f"Processed session_end for UID '{session_uid}'. Deleted speaker events and session start cache from Redis (count: {deleted_count})."
                        )
                        ...
                        logger.error(
                            f"Redis error deleting keys for UID '{session_uid}' on session_end: {e_redis}"
                        )
                        return False  # Retryable Redis error
                    return True  # Successfully processed session_end
                else:
                    logger.warning(
                        f"Message {message_id} has unknown type '{message_type}'. Skipping."
                    )
                    return True

            except ValueError as ve:  # Raised by get_user_by_token or other validation
                logger.warning(
                    f"Auth/Lookup or validation failed for message {message_id}: {ve}. Skipping."
                )
                return True
            except Exception as db_err:
                logger.error(
                    f"DB/Lookup error preparing for message {message_id}: {db_err}",
                    exc_info=True,
                )
                await db.rollback()
                return False
            ...
            if not all(field in stream_data for field in required_fields_transcription):
                logger.warning(
                    f"Transcription message {message_id} payload missing 'segments' field. Skipping. Payload: {payload_json[:200]}..."
                )
                return True
            ...
            try:
                if session_uid_from_payload:
                    ...
                    session_start_cache_key = (
                        f"meeting_session:{session_uid_from_payload}:start"
                    )
                    ...
                    if cached_start:
                        try:
                            cached_str = (
                                cached_start
                                if isinstance(cached_start, str)
                                else cached_start.decode("utf-8")
                            )
                            ...
                            logger.debug(
                                f"[Msg {message_id}/Meet {internal_meeting_id}] Loaded session start from Redis cache for UID {session_uid_from_payload}"
                            )
                        except Exception as cache_parse_err:
                            logger.warning(
                                f"[Msg {message_id}/Meet {internal_meeting_id}] Failed to parse cached session start: {cache_parse_err}"
                            )
                    ...
                    if not session_start_utc:
                        ...
                        if session_row and getattr(
                            session_row, "session_start_time", None
                        ):
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"[Msg {message_id}/Meet {internal_meeting_id}] Loaded session start from DB and cached for UID {session_uid_from_payload}"
                                )
                            except Exception:
                                pass
            except Exception as _sess_err:
                logger.warning(
                    f"[Msg {message_id}/Meet {internal_meeting_id}] Unable to resolve session start time for UID {session_uid_from_payload}: {_sess_err}"
                )

            if not session_uid_from_payload:
                logger.warning(
                    f"[Msg {message_id}/Meet {internal_meeting_id}] Message missing 'uid' for transcription segments. Cannot map speakers. Segments in this message will not have speaker info."
                )

            for i, segment in enumerate(stream_data.get("segments", [])):
                if (
                    not isinstance(segment, dict)
                    or segment.get("start") is None
                    or segment.get("end") is None
                ):
                    logger.warning(
                        f"[Msg {message_id}/Meet {internal_meeting_id}] Skipping segment {i} missing structure or 'start'/'end': {segment}"
                    )
                    continue
                try:
                    ...
                    logger.warning(
                        f"[Msg {message_id}/Meet {internal_meeting_id}] Skipping segment {i} invalid time format: {time_err} - Segment: {segment}"
                    )
                    continue
                ...
                if end_time_float < start_time_float:
                    ...
                    logger.warning(
                        f"[Msg {message_id}/Meet {internal_meeting_id}] Corrected inverted times to start={start_time_float}, end={end_time_float}"
                    )

                # Skip zero/negative duration segments
                if end_time_float - start_time_float < 1e-3:
                    logger.debug(
                        f"[Msg {message_id}/Meet {internal_meeting_id}] Skipping ~zero-length segment: {segment}"
                    )
                    continue
                ...
                if session_uid_from_payload:
                    ...
                    mapping_status = mapping_result.get(
                        "status", STATUS_ERROR
                    )  # Default to STATUS_ERROR if not present
                else:
                    ...
                    logger.warning(
                        f"[Msg {message_id}/Meet {internal_meeting_id}/Seg {start_time_key}] No session_uid_from_payload. Cannot map speakers."
                    )
                ...
                if session_start_utc is not None:
                    try:
                        abs_start_dt = session_start_utc + timedelta(
                            seconds=start_time_float
                        )
                        abs_end_dt = session_start_utc + timedelta(
                            seconds=end_time_float
                        )
                        abs_start_iso = abs_start_dt.isoformat()
                        abs_end_iso = abs_end_dt.isoformat()
                    except Exception as _abs_err:
                        logger.debug(
                            f"[Msg {message_id}/Meet {internal_meeting_id}] Failed to compute absolute times: {_abs_err}"
                        )
                ...
                try:
                    ...
                    if existing_json:
                        ...
                        existing_norm = {
                            ...
                            "end_time": round(
                                float(existing_data.get("end_time", 0)), 3
                            ),
                            "absolute_start_time": existing_data.get(
                                "absolute_start_time"
                            ),
                        ...
                        if existing_norm == new_norm:
                            ...
                            logger.debug(
                                f"[Msg {message_id}/Meet {internal_meeting_id}/Seg {start_time_key}] No change detected, skipping."
                            )
                            continue
                except Exception as _cmp_err:
                    logger.debug(
                        f"[Msg {message_id}/Meet {internal_meeting_id}] Change comparison failed: {_cmp_err}; treating as changed"
                    )
            ...
            if segment_count > 0:
                try:
                    async with redis_c.pipeline(transaction=True) as pipe:
                        ...
                        if any(
                            res is None for res in results
                        ):  # Simplified critical failure check
                            logger.error(
                                f"Redis pipeline command failed critically for message {message_id}. Results: {results}"
                            )
                            return False
                        logger.info(
                            f"Stored/Updated {segment_count} segments in Redis from message {message_id} for meeting {internal_meeting_id}. Results: {results}"
                        )
                except redis.exceptions.RedisError as redis_err:
                    logger.error(
                        f"Redis pipeline error storing segments for message {message_id}: {redis_err}",
                        exc_info=True,
                    )
                    return False
                except Exception as pipe_err:
                    logger.error(
                        f"Unexpected pipeline error storing segments for message {message_id}: {pipe_err}",
                        exc_info=True,
                    )
                    return False
                ...
                if changed_segments:
                    try:
                        ...
                        logger.info(
                            f"Published {len(changed_segments)} changed segments to {channel}"
                        )
                    except Exception as pub_err:
                        logger.error(
                            f"Failed to publish mutable transcript update for meeting {internal_meeting_id}: {pub_err}"
                        )
                else:
                    logger.debug(
                        f"No changed segments to publish for meeting {internal_meeting_id} from message {message_id}"
                    )
            else:
                logger.info(
                    f"No valid segments found in message {message_id} for meeting {internal_meeting_id} to store in Redis."
                )
            return True

    except json.JSONDecodeError as e:
        logger.error(
            f"Failed to parse JSON payload for message {message_id}: {e}. Payload: {payload_json[:200]}... Acking to avoid loop."
        )
        return True
    except Exception as e:
        logger.error(
            f"Unexpected error in process_stream_message for {message_id}: {e}",
            exc_info=True,
        )
        return False


async def process_speaker_event_message(
    message_id: str, event_data: Dict[str, Any], redis_c: aioredis.Redis
) -> bool:
    ...
    try:
        ...
        required_fields = [
            "uid",
            "relative_client_timestamp_ms",
            "event_type",
            "participant_name",
        ]
        if not all(field in event_data for field in required_fields):
            logger.warning(
                f"[SpeakerProcessor] Speaker event message {message_id} missing required fields. Skipping. Data: {event_data}"
            )
        ...
        try:
            ...
            logger.warning(
                f"[SpeakerProcessor] Invalid relative_client_timestamp_ms '{event_data['relative_client_timestamp_ms']}' for message {message_id}. Skipping."
            )
            return True  # Bad data, OK to ACK
        ...
        logger.debug(
            f"[SpeakerProcessor] Stored speaker event for UID '{session_uid}' at {relative_timestamp_ms}ms. Key: {sorted_set_key}. Message ID: {message_id}"
        )
        return True

    except (
        json.JSONDecodeError
    ) as json_err:  # Should not happen if data is already dict
        logger.error(
            f"[SpeakerProcessor] Error serializing speaker event payload to JSON for message {message_id}: {json_err}. Data: {event_data}"
        )
        return True  # Cannot process, but ack to avoid loop with bad data format.
    except redis.exceptions.RedisError as e_redis:
        logger.error(
            f"[SpeakerProcessor] Redis error processing speaker event message {message_id}: {e_redis}",
            exc_info=True,
        )
        return False  # Potentially recoverable Redis error, DO NOT ACK
    except Exception as e:
        logger.error(
            f"[SpeakerProcessor] Unexpected error in process_speaker_event_message for {message_id}: {e}",
            exc_info=True,
        )
        return False  # Unexpected error, DO NOT ACK
