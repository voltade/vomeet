...
export async function callStatusChangeCallback(
	...
	log(
		`🔥 UNIFIED CALLBACK: ${status.toUpperCase()} - reason: ${reason || "none"}`,
	);

	if (!botConfig.botManagerCallbackUrl) {
		log(
			"Warning: No bot manager callback URL configured. Cannot send status change callback.",
		);
		return;
	}
	...
	if (!botConfig.connectionId) {
		log(
			"Warning: No connection ID configured. Cannot send status change callback.",
		);
		return;
	}
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		...
		try {
			...
			const baseUrl = botConfig.botManagerCallbackUrl.replace(
				"/exited",
				"/status_change",
			);
			...
			log(
				`Sending unified status change callback to ${baseUrl} (attempt ${attempt + 1}/${maxRetries})`,
			);
			...
			if (response.ok) {
				...
				if (
					...
					log(
						`${status} status change callback sent and processed successfully`,
					);
					return; // Success, exit retry loop
				} else {
					log(
						`Callback returned unexpected status: ${responseBody.status}, detail: ${responseBody.detail || "none"}`,
					);
			...
			log(
				`Callback attempt ${attempt + 1} failed: ${isTimeout ? "timeout after 5s" : error.message}`,
			);
			...
			if (attempt < maxRetries - 1) {
				...
				log(
					`All ${maxRetries} callback attempts failed. Bot-manager may not have received the status change.`,
				);
			}
		}
	}
