...
export async function callAwaitingAdmissionCallback(
	botConfig: any,
): Promise<void> {
	await callStatusChangeCallback(botConfig, "awaiting_admission");
}

export async function callLeaveCallback(
	_botConfig: any,
	...
	log(
		`Leave callback requested with reason: ${reason} - handled by exit callback`,
	);
}
