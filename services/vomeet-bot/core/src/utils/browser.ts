		...
		mediaElements.forEach((element: any, index: number) => {
			try {
				...
				if (
					...
					sourcesConnected++;
					(window as any).logBot(
						`Connected audio stream from element ${index + 1}/${mediaElements.length}. Tracks=${elementStream.getAudioTracks().length}`,
					);
				} else {
					(window as any).logBot(
	...
	setupAudioDataProcessor(
		...
		this.processor.recorder.onaudioprocess = async (event: any) => {
			...
			if (this.processor?.sessionAudioStartTimeMs === null) {
				...
				(window as any).logBot(
					`[Audio] Session audio start time set: ${this.processor?.sessionAudioStartTimeMs}`,
				);
			}
...
export class BrowserWhisperLiveService {
	...
	private async simpleConnection(): Promise<WebSocket | null> {
		try {
			...
			this.socket.onopen = () => {
				...
				(window as any).logBot(
					`[Failover] WebSocket connection opened successfully to ${this.whisperLiveUrl}. New UID: ${this.currentUid}. Lang: ${this.botConfigData.language}, Task: ${this.botConfigData.task}`,
				);
				...
				(window as any).logBot(
					`Sending initial config message: ${JSON.stringify(configPayload)}`,
				);
			...
			(window as any).logBot(
				`[WhisperLive] Connection error: ${error.message}`,
			);
			return null;
		}
	}
	...
	private async attemptConnection(): Promise<WebSocket | null> {
		try {
			(window as any).logBot(
				`[STUBBORN] 🚀 Connecting to WhisperLive with NEVER-GIVE-UP reconnection: ${this.whisperLiveUrl} (attempt ${this.retryCount + 1})`,
			);
			...
			this.socket.onopen = (_event) => {
				(window as any).logBot(
					`[STUBBORN] ✅ WebSocket CONNECTED to ${this.whisperLiveUrl}! Retry count reset from ${this.retryCount}.`,
				);
			...
			this.socket.onerror = (event) => {
				(window as any).logBot(
					`[STUBBORN] ❌ WebSocket ERROR. Will start stubborn reconnection...`,
				);
			...
			this.socket.onclose = (event) => {
				(window as any).logBot(
					`[STUBBORN] ❌ WebSocket CLOSED. Code: ${event.code}, Reason: "${event.reason}". WILL RECONNECT NO MATTER WHAT!`,
				);
			...
			(window as any).logBot(
				`[STUBBORN] ❌ Connection creation error: ${error.message}. WILL KEEP TRYING!`,
			);
			this.startStubbornReconnection();
			return null;
		}
	...
	private startStubbornReconnection(): void {
		...
		const delay = Math.min(
			this.retryDelayMs * 1.5 ** Math.min(this.retryCount, 10),
			10000,
		);

		(window as any).logBot(
			`[STUBBORN] 🔄 Starting STUBBORN reconnection in ${delay}ms (attempt ${this.retryCount + 1}/∞ - WE NEVER GIVE UP!)...`,
		);
		...
		this.reconnectInterval = setTimeout(async () => {
			...
			if (this.retryCount >= 1000) {
				// Reset counter every 1000 attempts to prevent overflow
				(window as any).logBot(
					`[STUBBORN] 🔄 Resetting retry counter after 1000 attempts. WE WILL NEVER GIVE UP! EVER!`,
				);
			...
			if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
				(window as any).logBot(
					`[STUBBORN] 🔄 Attempting reconnection (retry ${this.retryCount})...`,
				);
	...
	sendAudioData(audioData: Float32Array): boolean {
		...
		try {
			...
			(window as any).logBot(
				`[WhisperLive] Error sending audio data: ${error.message}`,
			);
			return false;
		}
	}
	...
	sendAudioChunkMetadata(chunkLength: number, sampleRate: number): boolean {
		...
		try {
			...
			(window as any).logBot(
				`[WhisperLive] Error sending audio metadata: ${error.message}`,
			);
			return false;
		}
	}
	...
	sendSpeakerEvent(
		...
		try {
			...
			(window as any).logBot(
				`[WhisperLive] Error sending speaker event: ${error.message}`,
			);
			return false;
		}
	}
	...
	sendSessionControl(event: string, botConfigData: any): boolean {
		...
		try {
			...
			(window as any).logBot(
				`[WhisperLive] Error sending session control: ${error.message}`,
			);
			return false;
		}
	}
	...
	close(): void {
		(window as any).logBot(
			`[STUBBORN] 🛑 Closing WebSocket and stopping reconnection...`,
		);
