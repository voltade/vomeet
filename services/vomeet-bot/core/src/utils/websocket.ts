...
export class WebSocketManager {
		...
		try {
			...
			this.connectionTimeoutHandle = setTimeout(() => {
				if (this.socket && this.socket.readyState === WebSocket.CONNECTING) {
					log(
						`[WebSocket] Connection to ${url} timed out after ${this.config.connectionTimeoutMs}ms`,
					);
					this.socket.close();
				}
			...
			this.socket.onclose = (event) => {
				...
				this.connecting = false;
				log(
					`[WebSocket] Connection to ${url} closed. Code: ${event.code}, Reason: ${event.reason}`,
				);
			...
			this.connecting = false;
			log(
				`[WebSocket] Critical error creating WebSocket connection: ${error.message}`,
			);
	...
	private async retryConnection(
		...
		if (this.retryCount >= this.config.maxRetries) {
			log(
				`[WebSocket] Max retries (${this.config.maxRetries}) reached for ${url}`,
			);
			return;
		}

		this.retryCount++;
		const delay = this.config.retryDelayMs * 2 ** (this.retryCount - 1);

		log(
			`[WebSocket] Retrying connection to ${url} in ${delay}ms (attempt ${this.retryCount}/${this.config.maxRetries})`,
		);
	...
	private async retryWithNextCandidate(
		...
		try {
			...
			if (nextUrl) {
				log(
					`[WebSocket] Got next candidate: ${nextUrl}. Retrying in ${this.config.retryDelayMs}ms`,
				);
				setTimeout(() => {
