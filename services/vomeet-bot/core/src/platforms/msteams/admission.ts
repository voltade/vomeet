		...
		for (const selector of teamsRejectionIndicators) {
			try {
				...
				if (await element.isVisible()) {
					log(
						`🚨 Teams admission rejection detected: Found rejection indicator "${selector}"`,
					);
					return true;
				}
			} catch (_e) {}
...
export async function checkForAdmissionIndicators(
	page: Page,
): Promise<boolean> {
...
export async function waitForTeamsMeetingAdmission(
	...
	try {
		...
		const initialLobbyTextVisible = await page
			.locator(teamsWaitingRoomIndicators[0])
			.isVisible();

		// Use selector-based approach instead of getByRole for consistency
		const joinNowButtons = teamsJoinButtonSelectors.filter((sel) =>
			sel.includes("Join now"),
		);
		...
		if (
			initialLeaveButtonFound &&
			!initialLobbyTextVisible &&
			!initialJoinNowButtonVisible
		) {
			log(
				`Found Teams admission indicator: visible Leave button - Bot is already admitted to the meeting!`,
			);
			...
			log(
				"Successfully admitted to the Teams meeting - no waiting room required",
			);
			return true;
		}
		...
		const waitingLobbyTextVisible = await page
			.locator(teamsWaitingRoomIndicators[0])
			.isVisible();
		...
		if (waitingLobbyTextVisible || waitingJoinNowButtonVisible) {
			log(
				`Found Teams waiting room indicator: lobby text or Join now button visible - Bot is still in waiting room`,
			);
		...
		if (stillInWaitingRoom) {
			log(
				`Bot is in Teams waiting room. Waiting for ${timeout}ms for admission...`,
			);
			...
			while (Date.now() - startTime < timeout) {
				...
				const lobbyTextStillVisible = await page
					.locator(teamsWaitingRoomIndicators[0])
					.isVisible();
				...
				if (!stillWaiting) {
					log(
						"Teams waiting room indicator disappeared - checking if bot was admitted or rejected...",
					);
					...
					if (leaveButtonNowFound) {
						log(
							`✅ Bot was admitted to the Teams meeting: Leave button confirmed`,
						);
						return true;
					} else {
						log(
							"⚠️ Teams waiting room disappeared but no clear admission indicators found - assuming admitted",
						);
				...
				log(
					`Still in Teams waiting room... ${Math.round((Date.now() - startTime) / 1000)}s elapsed`,
				);
			}

			// After waiting, check if we're still in waiting room using visibility
			const finalLobbyTextVisible = await page
				.locator(teamsWaitingRoomIndicators[0])
				.isVisible();
			...
			const finalWaitingCheck =
				finalLobbyTextVisible || finalJoinNowButtonVisible;

			if (finalWaitingCheck) {
				throw new Error(
					"Bot is still in the Teams waiting room after timeout - not admitted to the meeting",
				);
			}
		}
		...
		log(
			"Checking for Teams meeting controls as primary admission indicator...",
		);
		...
		const admitted =
			finalLeaveButtonFound &&
			!finalLobbyTextVisible &&
			!finalJoinNowButtonVisible;

		if (admitted) {
			log(
				`Found Teams admission indicator: visible Leave button - Bot is admitted to the meeting`,
			);
		}
		...
		if (!admitted) {
			...
			log(
				"No Teams meeting indicators found - checking if bot was rejected before concluding failure...",
			);

			const isRejected = await checkForTeamsRejection(page);
			if (isRejected) {
				log(
					"🚨 Bot was rejected from the Teams meeting by admin (final check)",
				);
				throw new Error("Bot admission was rejected by meeting admin");
			}
			...
			log(
				"No rejection indicators found - bot likely failed to join or is in unknown state",
			);
			throw new Error(
				"Bot failed to join the Teams meeting - no meeting indicators found",
			);
		}
