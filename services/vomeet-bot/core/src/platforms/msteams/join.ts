...
import { callJoiningCallback, log } from "../../utils";
import {
...
export async function joinMicrosoftTeams(
	...
	await page.addInitScript(() => {
		try {
			...
			if (
				win.__vomeetRemoteAudioHookInstalled ||
			...
			win.__vomeetInjectedAudioElements =
				win.__vomeetInjectedAudioElements || [];
			...
			function wrapPeerConnection(this: any, ...args: any[]) {
				...
				const handleTrack = (event: RTCTrackEvent) => {
					try {
						...
						(win.__vomeetInjectedAudioElements as HTMLAudioElement[]).push(
							audioEl,
						);
						win.__vomeetCapturedRemoteAudioStreams =
							win.__vomeetCapturedRemoteAudioStreams || [];
						win.__vomeetCapturedRemoteAudioStreams.push(stream);

						win.logBot?.(
							`[Audio Hook] Injected remote audio element (track=${event.track.id}, readyState=${event.track.readyState}).`,
						);
					} catch (hookError) {
			...
			win.logBot?.(
				"[Audio Hook] RTCPeerConnection patched to mirror remote audio tracks.",
			);
		} catch (initError) {
	...
	try {
		...
		const dontUseAudioRadio = page
			...
			.first();
		const computerAudioVisible = await computerAudioRadio
			.isVisible()
			.catch(() => false);
		...
		const speakerOnButton = page
			.locator(teamsSpeakerEnableSelectors.join(", "))
			.first();
		const speakerOffButton = page
			.locator(teamsSpeakerDisableSelectors.join(", "))
			.first();

		const speakerOnVisible = await speakerOnButton
			.isVisible()
			.catch(() => false);
		const speakerOffVisible = await speakerOffButton
			.isVisible()
			.catch(() => false);
