			...
			if (typeof (window as any).performLeaveAction !== "function") {
				(window as any).performLeaveAction = async () => {
					try {
						...
						(window as any).logBot?.(
							"🔍 Starting stateless Teams leave button detection...",
						);
						(window as any).logBot?.(
							...
							try {
								...
								if (button) {
									...
									if (isVisible) {
										...
										(window as any).logBot?.(
											`✅ Found clickable button: aria-label="${ariaLabel}", data-tid="${dataTid}", text="${textContent}"`,
										);
...
export async function leaveMicrosoftTeams(
	page: Page | null,
	botConfig?: BotConfig,
