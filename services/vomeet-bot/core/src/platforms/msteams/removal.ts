		...
		for (const selector of teamsRemovalIndicators) {
			try {
				...
				if (await element.isVisible()) {
					log(
						`🚨 Teams removal detected: Found removal indicator "${selector}"`,
					);
					return true;
				}
			} catch (_e) {}
...
export function startTeamsRemovalMonitor(
	page: Page,
	...
	const removalCheckInterval = setInterval(async () => {
		try {
			...
			if (isRemoved && !removalDetected) {
				...
				log(
					"🚨 Teams removal detected from Node.js side. Initiating graceful shutdown...",
				);
				...
				try {
					...
					await page.evaluate(() => {
						...
						for (const b of btns) {
							...
							if (t === "dismiss" || a.includes("dismiss")) {
								clickIfVisible(b);
								break;
							}
						}
