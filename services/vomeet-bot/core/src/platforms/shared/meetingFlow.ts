...
import { callStartupCallback, log } from "../../utils";
...
export async function runMeetingFlow(
	...
	if (hasStopSignalReceived()) {
		log(
			"⛔ Stop signal detected before admission wait. Exiting without joining.",
		);
	...
	try {
				...
				.catch((error: any) => {
					...
					if (msg.includes("rejected by meeting admin")) {
						return {
							admitted: false,
							rejected: true,
							reason: "admission_rejected_by_admin",
		...
		const isAdmitted =
			...
			(typeof admissionResult === "object" &&
				!!(admissionResult as AdmissionDecision).admitted);
		if (!isAdmitted) {
			...
			if (decision.rejected) {
				await gracefulLeaveFunction(
					page,
					0,
					decision.reason || "admission_rejected_by_admin",
				);
				return;
			}
			...
			try {
				...
				if (result)
					log(
						"✅ Successfully performed graceful leave during admission timeout",
					);
			} catch {}
			...
			await gracefulLeaveFunction(
				page,
				0,
				decision.reason || "admission_timeout",
			);
			return;
		}
		...
		const stopRemoval = strategies.startRemovalMonitor(page, () => {
			if (signalRemoval) signalRemoval();
		});
		...
		try {
			...
			if (
				msg === tokens.leftAloneToken ||
				msg.includes(tokens.leftAloneToken)
			) {
				await gracefulLeaveFunction(page, 0, "left_alone_timeout");
				return;
			}
			if (
				msg === tokens.startupAloneToken ||
				msg.includes(tokens.startupAloneToken)
			) {
