		...
		for (const selector of googleRemovalIndicators) {
			try {
				...
				if (await element.isVisible()) {
					log(
						`🚨 Google Meet removal detected: Found removal indicator "${selector}"`,
					);
					return true;
				}
			} catch (_e) {}
...
export function startGoogleRemovalMonitor(
	page: Page,
	...
	const removalCheckInterval = setInterval(async () => {
		try {
			...
			if (isRemoved && !removalDetected) {
				...
				log(
					"🚨 Google Meet removal detected from Node.js side. Initiating graceful shutdown...",
				);
