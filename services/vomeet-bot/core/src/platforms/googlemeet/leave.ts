				...
				(window as any).performLeaveAction = async () => {
					try {
						...
						(window as any).logBot?.(
							"🔥 Calling leave callback before attempting to leave...",
						);
						try {
							...
							if (botConfig) {
								...
								(window as any).logBot?.(
									"📡 Leave callback will be sent from Node.js context",
								);
							}
						} catch (callbackError: any) {
							(window as any).logBot?.(
								`⚠️ Warning: Could not prepare leave callback: ${callbackError.message}`,
							);
						}
						...
						(window as any).logBot?.(
							"🔍 Starting stateless Google Meet leave button detection...",
						);
						(window as any).logBot?.(
							`📋 Will try ${leaveSelectors.length} selectors until one works`,
						);
						...
						for (let i = 0; i < leaveSelectors.length; i++) {
							...
							try {
								(window as any).logBot?.(
									`🔍 [${i + 1}/${leaveSelectors.length}] Trying selector: ${selector}`,
								);
								...
								if (button) {
									...
									if (isVisible) {
										...
										(window as any).logBot?.(
											`✅ Found clickable button: aria-label="${ariaLabel}", text="${textContent}"`,
										);
										...
										(window as any).logBot?.(
											`ℹ️ Button found but not visible for selector: ${selector}`,
										);
									}
								} else {
									(window as any).logBot?.(
										`ℹ️ No button found for selector: ${selector}`,
									);
								}
							} catch (e: any) {
								(window as any).logBot?.(
						...
						(window as any).logBot?.(
							"❌ No working leave/cancel button found - tried all selectors",
						);
						return false;
					} catch (err: any) {
						(window as any).logBot?.(
							`Error during Google Meet leave attempt: ${err.message}`,
						);
						return false;
					}
				};
...
export async function leaveGoogleMeet(
	page: Page | null,
	botConfig?: BotConfig,
	...
	if (botConfig) {
		try {
			...
			log(
				`⚠️ Warning: Failed to send leave callback: ${callbackError.message}. Continuing with leave attempt...`,
			);
		}
	} else {
