...
import { callAwaitingAdmissionCallback, log } from "../../utils";
import {
		...
		for (const selector of googleRejectionIndicators) {
			try {
				...
				if (await element.isVisible()) {
					log(
						`🚨 Google Meet admission rejection detected: Found rejection indicator "${selector}"`,
					);
					return true;
				}
			} catch (_e) {}
...
export async function checkForGoogleAdmissionIndicators(
	page: Page,
): Promise<boolean> {
...
export async function checkForWaitingRoomIndicators(
	page: Page,
): Promise<boolean> {
...
export async function waitForGoogleMeetingAdmission(
	...
	try {
		...
		await page.screenshot({
			path: "/app/storage/screenshots/bot-checkpoint-1-admission-start.png",
			fullPage: true,
		});
		...
		if (initialAdmissionFound && !initialLobbyStillVisible) {
			log(
				`Found Google Meet admission indicator: visible meeting controls - Bot is already admitted to the meeting!`,
			);

			// Take screenshot when already admitted
			await page.screenshot({
				path: "/app/storage/screenshots/bot-checkpoint-2-admitted.png",
				fullPage: true,
			});
			...
			try {
				...
				log(
					"Awaiting admission callback sent successfully (immediate admission)",
				);
			} catch (callbackError: any) {
				log(
					`Warning: Failed to send awaiting admission callback: ${callbackError.message}. Continuing...`,
				);
			}

			log(
				"Successfully admitted to the Google Meet meeting - no waiting room required",
			);
			return true;
		}

		log(
			"Bot not yet admitted - checking for Google Meet waiting room indicators...",
		);
		...
		if (waitingRoomVisible) {
			log(
				`Found Google Meet waiting room indicator - Bot is still in waiting room`,
			);

			// Take screenshot when waiting room indicator found
			await page.screenshot({
				path: "/app/storage/screenshots/bot-checkpoint-4-waiting-room.png",
				fullPage: true,
			});
			...
			try {
				...
				log(
					`Warning: Failed to send awaiting admission callback: ${callbackError.message}. Continuing with admission wait...`,
				);
			}
		...
		if (stillInWaitingRoom) {
			log(
				`Bot is in Google Meet waiting room. Waiting for ${timeout}ms for admission...`,
			);
			...
			while (Date.now() - startTime < timeout) {
				...
				if (!stillWaiting) {
					log(
						"Google Meet waiting room indicator disappeared - checking if bot was admitted or rejected...",
					);
					...
					if (admissionFound) {
						log(
							`✅ Bot was admitted to the Google Meet meeting: meeting controls confirmed`,
						);
						return true;
					}
				...
				log(
					`Still in Google Meet waiting room... ${Math.round((Date.now() - startTime) / 1000)}s elapsed`,
				);
			}
			...
			if (finalWaitingCheck) {
				throw new Error(
					"Bot is still in the Google Meet waiting room after timeout - not admitted to the meeting",
				);
			}
		} else {
			// Not in waiting room and not admitted yet: actively poll during the timeout
			log(
				`No waiting room detected. Polling for admission for up to ${timeout}ms...`,
			);
			...
			while (Date.now() - startTime < timeout) {
				...
				if (isRejected) {
					log(
						"🚨 Bot was rejected from the Google Meet meeting by admin (polling mode)",
					);
					throw new Error("Bot admission was rejected by meeting admin");
				}
				...
				if (admissionFound && !lobbyVisible) {
					log(
						"✅ Bot admitted during polling window (meeting controls visible)",
					);
					return true;
				}
				...
				if (lobbyVisible) {
					log(
						"ℹ️ Waiting room appeared during polling. Switching to waiting-room monitoring...",
					);
					...
					try {
						await callAwaitingAdmissionCallback(botConfig);
						log(
							"Awaiting admission callback sent successfully (during polling)",
						);
					} catch (callbackError: any) {
						log(
							`Warning: Failed to send awaiting admission callback: ${callbackError.message}. Continuing...`,
				...
				await page.waitForTimeout(checkInterval);
				log(
					`Polling for Google Meet admission... ${Math.round((Date.now() - startTime) / 1000)}s elapsed`,
				);
			}
			...
			if (stillInWaitingRoom) {
				...
				while (Date.now() - startTime2 < timeout) {
					...
					if (!stillWaiting) {
						...
						if (isRejected2)
							throw new Error("Bot admission was rejected by meeting admin");
						const admissionFound2 =
							await checkForGoogleAdmissionIndicators(page);
		...
		log(
			"No admission indicators after timeout - checking rejection one last time...",
		);
		...
		await page.screenshot({
			path: "/app/storage/screenshots/bot-checkpoint-3-no-indicators.png",
			fullPage: true,
		});
		log("📸 Screenshot taken: No meeting indicators found after timeout");
		throw new Error(
			"Bot failed to join the Google Meet meeting - no meeting indicators found within timeout",
		);
	} catch (error: any) {
		throw new Error(
