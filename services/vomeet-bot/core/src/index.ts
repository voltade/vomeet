...
import { browserArgs, userAgent } from "./constants";
...
import {
	...
	mapExitReasonToStatus,
} from "./services/unified-callback";
...
const handleRedisMessage = async (
	...
	log(
		`[DEBUG] handleRedisMessage entered for channel ${channel}. Message: ${message.substring(0, 100)}...`,
	);
	// ++++++++++++++++++++++++++++++++++
	...
	try {
		...
		if (command.meeting_id && command.meeting_id !== meetingId) {
			log(
				`⚠️ Ignoring command for different meeting: ${command.meeting_id} (ours: ${meetingId})`,
			);
			return;
		}
				...
				try {
					await page.evaluate(
						([lang, task]) => {
							const tryApply = () => {
								...
								if (typeof fn === "function") {
									try {
										...
										console.error(
											"[Reconfigure] Error invoking triggerWebSocketReconfigure:",
											e?.message || e,
										);
									}
									return true;
							...
							if (!tryApply()) {
								console.warn(
									"[Reconfigure] triggerWebSocketReconfigure not ready. Retrying for up to 15s...",
								);
								...
								try {
									const ev = new CustomEvent("vomeet:reconfigure", {
										detail: { lang, task },
									});
					...
					log(
						"Sent reconfigure command to browser context (with retry if not yet ready).",
					);
				} catch (evalError: any) {
					log(
...
async function performGracefulLeave(
	...
	if (botManagerCallbackUrl && currentConnectionId) {
		...
		const statusMapping = mapExitReasonToStatus(
			finalCallbackReason,
		...
		try {
			...
			log(
				`[Graceful Leave] Error sending unified exit callback: ${callbackError.message}`,
			);
		}
	} else {
		log(
	...
	if (botConfig.platform === "teams") {
		...
		try {
			await context.addInitScript({
				path: require("node:path").join(__dirname, "browser-utils.global.js"),
			});
		} catch (e) {
