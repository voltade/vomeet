...
from shared_models.models import (
    ...
    MeetingSession,
    Transcription,
)  # <--- ADD MeetingSession and Transcription import
...
async def update_meeting_status(
    ...
    if current_status == new_status:
        logger.info(
            f"Meeting {meeting.id} already in status '{current_status.value}', treating transition as no-op"
        )
        return True
    ...
    if not is_valid_status_transition(current_status, new_status):
        logger.warning(
            f"Invalid status transition from '{current_status.value}' to '{new_status.value}' for meeting {meeting.id}"
        )
        return False
    ...
    logger.info(
        f"Meeting {meeting.id} status updated from '{old_status}' to '{new_status.value}'"
    )
    return True
...
def mint_meeting_token(
    ...
    signature = hmac.new(
        secret.encode("utf-8"), signing_input, digestmod="sha256"
    ).digest()
...
async def publish_meeting_status_change(
    ...
    if not redis_client:
        logger.warning(
            "Redis client not available for publishing meeting status change"
        )
        return
    try:
        payload = {
        ...
        logger.error(
            f"Failed to publish meeting status change for meeting {meeting_id}: {e}"
        )
...
async def schedule_status_webhook_task(
    ...
    logger.info(
        f"Scheduled status webhook task for meeting {meeting.id} status change: {old_status} -> {new_status}"
    )
...
class BotExitCallbackPayload(BaseModel):
    connection_id: str = Field(
        ..., description="The connectionId (session_uid) of the exiting bot."
    )
    exit_code: int = Field(
        ...,
        description="The exit code of the bot process (0 for success, 1 for UI leave failure).",
    )
    reason: Optional[str] = Field(
        "self_initiated_leave", description="Reason for the exit."
    )
    error_details: Optional[Dict[str, Any]] = Field(
        None,
        description="Detailed error information including stack trace, error message, and context.",
    )
    platform_specific_error: Optional[str] = Field(
        None, description="Platform-specific error message or details."
    )
    completion_reason: Optional[MeetingCompletionReason] = Field(
        None, description="Reason for completion if applicable."
    )
    failure_stage: Optional[MeetingFailureStage] = Field(
        None, description="Stage where failure occurred if applicable."
    )
...
class BotStatusChangePayload(BaseModel):
    ...
    container_id: Optional[str] = Field(
        None, description="The container ID of the bot."
    )
    ...
    platform_specific_error: Optional[str] = Field(
        None, description="Platform-specific error message."
    )
    completion_reason: Optional[MeetingCompletionReason] = Field(
        None, description="Reason for completion if applicable."
    )
    ...
    timestamp: Optional[str] = Field(
        None, description="Timestamp of the status change."
    )
...
async def startup_event():
    ...
    try:
        ...
        logger.error(
            f"Failed to initialize Docker client on startup: {e}", exc_info=True
        )
    ...
    logger.info(
        "Database, Docker Client (attempted), and Redis Client (attempted) initialized."
    )
...
async def _delayed_container_stop(
    container_id: str, meeting_id: int, delay_seconds: int = 30
):
    """
    ...
    """
    logger.info(
        f"[Delayed Stop] Task started for container {container_id} (meeting {meeting_id}). Waiting {delay_seconds}s before stopping."
    )
    await asyncio.sleep(delay_seconds)
    logger.info(
        f"[Delayed Stop] Delay finished for {container_id}. Attempting synchronous stop..."
    )
    try:
        ...
        logger.error(
            f"[Delayed Stop] Error stopping container {container_id}: {e}",
            exc_info=True,
        )
    ...
    try:
        ...
        logger.info(
            f"[Delayed Stop] Waiting {grace_period}s grace period for pending callbacks before finalizing meeting {meeting_id}..."
        )
        ...
        async with async_session_local() as db:
            ...
            if not meeting:
                logger.warning(
                    f"[Delayed Stop] Meeting {meeting_id} not found in DB. Cannot finalize."
                )
                return
            ...
            if meeting.status not in terminal_states:
                ...
                if success:
                    ...
                    logger.info(
                        f"[Delayed Stop] Meeting {meeting_id} finalized to COMPLETED and post-meeting tasks scheduled."
                    )
                else:
                    logger.error(
                        f"[Delayed Stop] Failed to finalize meeting {meeting_id} to COMPLETED."
                    )
            else:
                logger.info(
                    f"[Delayed Stop] Meeting {meeting_id} already in terminal state '{meeting.status}'. No finalization needed."
                )
    except Exception as e:
        logger.error(
            f"[Delayed Stop] Error during safety finalizer for meeting {meeting_id}: {e}",
            exc_info=True,
        )
...
async def request_bot(
    ...
    logger.info(
        f"Received bot request for platform '{req.platform.value}' with native ID '{req.native_meeting_id}' from user {current_user.id}"
    )
    ...
    if not constructed_url:
        logger.error(
            f"Invalid meeting URL for platform {req.platform.value} and ID {native_meeting_id}. Rejecting request."
        )
        raise HTTPException(
    ...
    existing_meeting_stmt = (
        ...
        .where(
            ...
            Meeting.status.in_(
                ["requested", "active"]
            ),  # Do NOT block on 'stopping' to allow immediate new bot
        )
    ...
    if user_limit > 0:
        ...
        if active_count >= user_limit:
            logger.warning(
                f"User {current_user.id} reached concurrent bot limit {active_count}/{user_limit}. Rejecting new launch."
            )
            raise HTTPException(
    ...
    if existing_meeting is None:
        ...
        try:
            ...
            logger.info(
                f"Published initial meeting.status 'requested' for meeting {meeting_id_for_bot}"
            )
        except Exception as _pub_err:
            logger.warning(
                f"Failed to publish initial 'requested' status for meeting {meeting_id_for_bot}: {_pub_err}"
            )
        ...
        logger.error(
            f"Logic error: Should have raised 409 for existing meeting {existing_meeting.id}, but proceeding."
        )
    ...
    if "new_meeting" in locals() and new_meeting is not None:
        ...
        raise HTTPException(
            ...
            detail="Internal server error preparing bot launch.",
        )

    meeting_id = (
        current_meeting_for_bot_launch.id
    )  # Internal DB ID for the bot being launched.
    ...
    try:
        logger.info(
            f"Attempting to start bot container for meeting {meeting_id} (native: {native_meeting_id})..."
        )
        ...
        logger.info(
            f"Scheduled background task to record session start for meeting {meeting_id}, session {connection_id}"
        )
        ...
        logger.info(
            f"Setting container ID {container_id} for meeting {meeting_id} (status remains 'requested' until bot confirms startup)"
        )
        ...
        logger.info(
            f"Successfully set container ID for meeting {meeting_id}. Status remains 'requested' until bot startup callback."
        )
        ...
        logger.warning(
            f"HTTPException occurred during bot startup for meeting {meeting_id}: {http_exc.status_code} - {http_exc.detail}"
        )
        try:
            # Fetch again or use current_meeting_for_bot_launch if it's the correct one to update
            meeting_to_update = await db.get(
                Meeting, meeting_id
            ...
            if meeting_to_update and meeting_to_update.status not in [
                ...
                logger.warning(
                    f"Updating meeting {meeting_id} status to 'error' due to HTTPException {http_exc.status_code}."
                )
                ...
                logger.error(
                    f"Could not find meeting {meeting_id} to update status to error after HTTPException."
                )
        except Exception as db_err:
            logger.error(
                f"Failed to update meeting {meeting_id} status to error after HTTPException: {db_err}"
            )
        raise http_exc

    except Exception as e:
        logger.error(
            f"Unexpected exception occurred during bot startup process for meeting {meeting_id} (after DB creation): {e}",
            exc_info=True,
        )
        try:
            ...
            if meeting_to_update and meeting_to_update.status not in [
                ...
                logger.warning(
                    f"Updating meeting {meeting_id} status to 'error' due to unexpected exception."
                )
                meeting_to_update.status = "error"
                if container_id:
                    meeting_to_update.bot_container_id = container_id
                ...
                logger.error(
                    f"Could not find meeting {meeting_id} to update status to error after unexpected exception."
                )
        except Exception as db_err:
            logger.error(
                f"Failed to update meeting {meeting_id} status to error after unexpected exception: {db_err}"
            )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "status": "error",
                "message": f"An unexpected error occurred during bot startup: {str(e)}",
                "meeting_id": meeting_id,
            },
        )
...
async def update_bot_config(
    ...
    logger.info(
        f"User {current_user.id} requesting config update for {platform.value}/{native_meeting_id}: lang={req.language}, task={req.task}"
    )
    ...
    active_meeting_stmt = (
        ...
        .where(
            ...
            Meeting.platform_specific_id == native_meeting_id,
            Meeting.status
            == MeetingStatus.ACTIVE.value,  # Must be active to reconfigure
        )
    ...
    if not active_meeting:
        logger.warning(
            f"No active meeting found for user {current_user.id}, {platform.value}/{native_meeting_id} to reconfigure."
        )
    ...
    logger.info(
        f"[DEBUG] Found active meeting record with internal ID: {internal_meeting_id}"
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully published reconfigure command for meeting {internal_meeting_id}."
        )
    except Exception as e:
        logger.error(
            f"Failed to publish reconfigure command to Redis channel {channel}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def stop_bot(
    ...
    logger.info(
        f"Received stop request for {platform_value}/{native_meeting_id} from user {current_user.id}"
    )
    ...
    if not all_meetings:
        logger.warning(
            f"Stop request: No meeting found for {platform_value}/{native_meeting_id} for user {current_user.id}"
        )
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No meeting found to stop."
        )
    ...
    if not non_terminal_meetings:
        ...
        logger.info(
            f"Stop request: Meeting {meeting.id} already in terminal state '{meeting.status}'. Returning 202 idempotently."
        )
    ...
    for meeting in non_terminal_meetings:
        ...
        if not meeting.bot_container_id:
            logger.info(
                f"Stop request: Meeting {meeting.id} has no container ID (status: {meeting.status}). Finalizing immediately."
            )
            success = await update_meeting_status(
                meeting,
                MeetingStatus.COMPLETED,
            ...
            logger.info(
                f"Scheduling post-meeting tasks for meeting {meeting.id} (no container case)."
            )
            background_tasks.add_task(run_all_tasks, meeting.id)
            continue

        logger.info(
            f"Found meeting {meeting.id} (status: {meeting.status}) with container {meeting.bot_container_id} for stop request."
        )

        # --- SIMPLE FAST-PATH: If very recent and pre-active, finalize immediately and kill container ---
        try:
            seconds_since_created = (
                (datetime.utcnow() - meeting.created_at).total_seconds()
        ...
        if meeting.status in [
            ...
            MeetingStatus.JOINING.value,
            MeetingStatus.AWAITING_ADMISSION.value,
        ] and (seconds_since_created is not None and seconds_since_created < 5):
            logger.info(
            ...
            background_tasks.add_task(
                _delayed_container_stop, meeting.bot_container_id, meeting.id, 0
            )
        ...
        if not redis_client:
            ...
            try:
                ...
                logger.info(
                    f"Publishing leave command to Redis channel '{command_channel}': {payload}"
                )
                await redis_client.publish(command_channel, payload)
                logger.info(
                    f"Successfully published leave command for meeting {meeting.id}."
                )
            except Exception as e:
                logger.error(
                    f"Failed to publish leave command to Redis channel {command_channel}: {e}",
                    exc_info=True,
                )
        ...
        logger.info(
            f"Scheduling delayed stop task for container {meeting.bot_container_id} (meeting {meeting.id})."
        )
        # Pass container_id, meeting_id, and delay
        background_tasks.add_task(
            _delayed_container_stop, meeting.bot_container_id, meeting.id, 30
        )
        ...
        logger.info(
            f"Stop request accepted for meeting {meeting.id}. Bot will transition to completed/failed via callback."
        )
        ...
        logger.info(
            f"Stop request for meeting {meeting.id} accepted. Leave command sent, delayed stop scheduled."
        )
...
async def get_user_bots_status(
    ...
    try:
        ...
        logger.error(
            f"Error fetching bot status for user {user_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bot_exit_callback(
    ...
    """
    logger.info(
        f"Received bot exit callback: connection_id={payload.connection_id}, exit_code={payload.exit_code}, reason={payload.reason}"
    )
    ...
    try:
        ...
        session_stmt = select(MeetingSession).where(
        ...
        if not meeting_session:
            logger.error(
                f"Bot exit callback: Could not find meeting session for connection_id {session_uid}. Cannot update meeting status."
            )
        ...
        logger.info(
            f"Bot exit callback: Found meeting_id {meeting_id} for connection_id {session_uid}"
        )
        ...
        if not meeting:
            logger.error(
                f"Bot exit callback: Found session but could not find meeting {meeting_id} itself."
            )
        ...
        if exit_code == 0:
            ...
            provided_reason = (
                payload.completion_reason or MeetingCompletionReason.STOPPED
            )
            transition_meta = {"exit_code": exit_code}
            if payload.platform_specific_error:
                transition_meta["platform_specific_error"] = (
                    payload.platform_specific_error
                )
            success = await update_meeting_status(
                meeting,
                MeetingStatus.COMPLETED,
            ...
            if success:
                ...
                logger.info(
                    f"Bot exit callback: Meeting {meeting_id} status updated to 'completed'."
                )
            else:
                logger.error(
                    f"Bot exit callback: Failed to update meeting {meeting_id} status to 'completed'"
                )
            ...
            if success:
                ...
                logger.warning(
                    f"Bot exit callback: Meeting {meeting_id} status updated to 'failed' due to exit_code {exit_code}."
                )
            else:
                logger.error(
                    f"Bot exit callback: Failed to update meeting {meeting_id} status to 'failed'"
                )
                return {"status": "error", "detail": "Failed to update meeting status"}
            ...
            if payload.error_details or payload.platform_specific_error:
                ...
                logger.info(
                    f"Bot exit callback: Stored error details in meeting {meeting_id} data: {error_data}"
                )
        ...
        await db.refresh(meeting)
        logger.info(
            f"Bot exit callback: Meeting {meeting.id} successfully updated in DB."
        )
        ...
        logger.info(
            f"Bot exit callback: Scheduling post-meeting tasks for meeting {meeting.id}."
        )
        background_tasks.add_task(run_all_tasks, meeting.id)
        ...
        if exit_code != 0 and meeting.bot_container_id:
            logger.warning(
                f"Bot exit callback: Scheduling delayed stop for container {meeting.bot_container_id} of failed meeting {meeting.id}."
            )
            background_tasks.add_task(
                _delayed_container_stop, meeting.bot_container_id, meeting.id, 10
            )
        ...
        logger.error(
            f"Bot exit callback: An unexpected error occurred: {e}", exc_info=True
        )
...
async def bot_startup_callback(
    payload: BotStartupCallbackPayload, db: AsyncSession = Depends(get_db)
):
    """
    ...
    """
    logger.info(
        f"Received bot startup callback: connection_id={payload.connection_id}, container_id={payload.container_id}"
    )
    ...
    try:
        ...
        if not meeting_session:
            logger.error(
                f"Bot startup callback: Could not find meeting session for connection_id {session_uid}. Cannot update meeting status."
            )
            return {"status": "error", "detail": "Meeting session not found"}

        meeting_id = meeting_session.meeting_id
        logger.info(
            f"Bot startup callback: Found meeting_id {meeting_id} for connection_id {session_uid}"
        )
        ...
        if not meeting:
            logger.error(
                f"Bot startup callback: Found session but could not find meeting {meeting_id} itself."
            )
            return {"status": "error", "detail": f"Meeting {meeting_id} not found"}
        ...
        if (
            ...
            logger.info(
                f"Bot startup callback: stop_requested set for meeting {meeting_id}. Ignoring startup transition."
            )
        ...
        if meeting.status in [
            ...
            if success:
                ...
                logger.error(
                    f"Bot startup callback: Failed to update meeting {meeting_id} status to 'active'"
                )
                return {"status": "error", "detail": "Failed to update meeting status"}
            ...
            await db.refresh(meeting)
            logger.info(
                f"Bot startup callback: Meeting {meeting_id} already active, updated container ID to {container_id}."
            )
        else:
            logger.warning(
                f"Bot startup callback: Meeting {meeting_id} has unexpected status '{meeting.status}', not updating."
            )
            return {
                "status": "warning",
                "detail": f"Meeting status '{meeting.status}' not updated",
            }
        ...
        if (
            ...
            and old_status != MeetingStatus.ACTIVE.value
        ):
            await publish_meeting_status_change(
                meeting.id,
                MeetingStatus.ACTIVE.value,
        ...
        logger.error(
            f"Bot startup callback: An unexpected error occurred: {e}", exc_info=True
        )
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bot_joining_callback(
    ...
    """
    logger.info(
        f"Received bot joining callback: connection_id={payload.connection_id}, container_id={payload.container_id}"
    )
    ...
    try:
        ...
        if not meeting_session:
            logger.error(
                f"Bot joining callback: Could not find meeting session for connection_id {session_uid}. Cannot update meeting status."
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        if not meeting:
            logger.error(
                f"Bot joining callback: Could not find meeting for session {meeting_session.meeting_id}. Cannot update meeting status."
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        if (
            ...
            logger.info(
                f"Bot joining callback: stop_requested set for meeting {meeting.id}. Ignoring joining transition."
            )
            return {"status": "ignored", "detail": "stop requested"}

        # Update meeting status to joining
        success = await update_meeting_status(
            meeting=meeting, new_status=MeetingStatus.JOINING, db=db
        )

        if success:
            logger.info(
                f"Bot joining callback: Successfully updated meeting {meeting.id} status to 'joining'"
            )
        ...
        logger.error(
            f"Bot joining callback: An unexpected error occurred: {e}", exc_info=True
        )
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bot_awaiting_admission_callback(
    ...
    """
    logger.info(
        f"Received bot awaiting admission callback: connection_id={payload.connection_id}, container_id={payload.container_id}"
    )
    ...
    try:
        ...
        if (
            ...
            logger.info(
                f"Bot awaiting admission callback: stop_requested set for meeting {meeting.id}. Ignoring waiting room transition."
            )
            return {"status": "ignored", "detail": "stop requested"}

        # Update meeting status to awaiting_admission
        success = await update_meeting_status(
            meeting=meeting, new_status=MeetingStatus.AWAITING_ADMISSION, db=db
        )

        if success:
            logger.info(
                f"Bot awaiting admission callback: Successfully updated meeting {meeting.id} status to 'awaiting_admission'"
            )
            # Publish status change to Redis
            await publish_meeting_status_change(
                meeting.id,
        ...
        logger.error(
            f"Bot awaiting admission callback: An unexpected error occurred: {e}",
            exc_info=True,
        )
        await db.rollback()
        raise HTTPException(
...
async def bot_status_change_callback(
    ...
    try:
        ...
        if not meeting_session:
            logger.error(
                f"Bot status change callback: Could not find meeting session for connection_id {session_uid}"
            )
            return {
                "status": "error",
                "detail": f"Meeting session not found for connection_id: {session_uid}",
        ...
        meeting_id = meeting_session.meeting_id
        logger.info(
            f"Bot status change callback: Found meeting_id {meeting_id} for connection_id {session_uid}"
        )
        ...
        if not meeting:
            logger.error(
                f"Bot status change callback: Could not find meeting {meeting_id}"
            )
            return {"status": "error", "detail": f"Meeting {meeting_id} not found"}
        ...
        if (
            ...
            logger.info(
                f"Bot status change callback: stop_requested set for meeting {meeting.id}. Ignoring {new_status.value} transition."
            )
            return {"status": "ignored", "detail": "stop requested"}
        ...
        if new_status == MeetingStatus.COMPLETED:
            ...
            success = await update_meeting_status(
                ...
                error_details=str(payload.error_details)
            ...
            if meeting.status in [
                ...
                await db.refresh(meeting)
                logger.info(
                    f"Bot status change callback: Meeting {meeting_id} already active, updated container ID to {payload.container_id}"
                )
                return {
                ...
                logger.warning(
                    f"Bot status change callback: Meeting {meeting_id} has unexpected status '{meeting.status}', not updating to active"
                )
                return {
                    "status": "warning",
                    "detail": f"Meeting status '{meeting.status}' not updated to active",
                }
            ...
            if not success:
                logger.error(
                    f"Bot status change callback: Failed to update meeting {meeting_id} status to '{new_status.value}'"
                )
                return {"status": "error", "detail": "Failed to update meeting status"}

        # Publish meeting status change via Redis Pub/Sub
        if success or (
            new_status == MeetingStatus.ACTIVE
        ...
        logger.error(
            f"Bot status change callback: An unexpected error occurred: {e}",
            exc_info=True,
        )
        await db.rollback()
        raise HTTPException(
