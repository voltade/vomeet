...
async def request_bot(
    ...
    Requires a valid Account API key (X-API-Key header).
    - Constructs the meeting URL from platform and native ID.
    - Creates a Meeting record in the database.
    - Starts a Docker container for the bot.
    - Returns the created Meeting details.
    """
    logger.info(
        f"Received bot request for platform '{req.platform.value}' with native ID '{req.native_meeting_id}' from account {account.id} ({account.name})"
    )
    ...
    if account_limit > 0:
        ...
        if active_count >= account_limit:
            logger.warning(
                f"Account {account.id} reached concurrent bot limit {active_count}/{account_limit}. Rejecting new launch."
            )
            raise HTTPException(
    ...
    try:
        ...
        logger.info(f"Published initial meeting.status 'requested' for meeting {meeting_id}")
    except Exception as _pub_err:
        logger.warning(f"Failed to publish initial 'requested' status for meeting {meeting_id}: {_pub_err}")

    # Preflight validation of required runtime inputs
    ...
    try:
        ...
        container_id, connection_id = await start_bot_container(
            ...
            user_token=account.api_key,  # Pass account API key as token
            native_meeting_id=native_meeting_id,
        ...
        # Set container ID, keep status as 'requested' until bot confirms startup
        logger.info(
...
async def update_bot_config(
    ...
    logger.info(
        f"Account {account.id} requesting config update for {platform.value}/{native_meeting_id}: lang={req.language}, task={req.task}"
    )

    # 1. Find the LATEST active meeting for this account/platform/native_id
    active_meeting_stmt = (
        select(Meeting)
        .where(
            Meeting.account_id == account.id,
    ...
    if not active_meeting:
        logger.warning(
            f"No active meeting found for account {account.id}, {platform.value}/{native_meeting_id} to reconfigure."
        )
...
async def stop_bot(
    ...
    logger.info(f"Received stop request for {platform_value}/{native_meeting_id} from account {account.id}")

    # Find all meetings matching the criteria
    stmt = (
        select(Meeting)
        .where(
            Meeting.account_id == account.id,
    ...
    if not all_meetings:
        logger.warning(
            f"Stop request: No meeting found for {platform_value}/{native_meeting_id} for account {account.id}"
        )
    ...
    non_terminal_meetings = [
        m for m in all_meetings if m.status not in [MeetingStatus.COMPLETED.value, MeetingStatus.FAILED.value]
    ]
    ...
    # Process each non-terminal meeting
    for meeting in non_terminal_meetings:
        # Handle meetings without container ID
        ...
        # Fast-path: If very recent and pre-active, finalize immediately and kill container
        try:
            seconds_since_created = (
                (datetime.utcnow() - meeting.created_at).total_seconds() if meeting.created_at else None
            )
        except Exception:
        ...
        # Schedule delayed container stop task
...
@app.get(
    ...
    summary="Get status of running bot containers for the authenticated account",
    dependencies=[Depends(get_account_from_api_key)],
)
async def get_account_bots_status(
    account: Account = Depends(get_account_from_api_key),
):
    """Retrieves a list of currently running bot containers associated with the account's API key."""
    logger.info(f"Fetching running bot status for account {account.id}")

    try:
        running_bots_list = await get_running_bots_status(account.id)
        return BotStatusResponse(running_bots=running_bots_list)
    except Exception as e:
        logger.error(f"Error fetching bot status for account {account.id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
