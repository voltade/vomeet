    ...
    if unix_socket_session is None:
        ...
        while retries < max_retries:
            try:
                ...
                logger.error(
                    f"Attempt {retries + 1}/{max_retries}: HTTP error communicating with Docker socket: {e}",
                    exc_info=True,
                )
                ...
                logger.error(
                    f"Attempt {retries + 1}/{max_retries}: Failed to initialize requests_unixsocket session: {e}",
                    exc_info=True,
                )
...
async def start_bot_container(
    ...
    create_payload = {
        ...
        "HostConfig": {"NetworkMode": DOCKER_NETWORK, "AutoRemove": True, "Mounts": []},
    }
    ...
    try:
        logger.info(
            f"Attempting to create bot container '{container_name}' ({BOT_IMAGE_NAME}) via socket ({socket_url_base})..."
        )
        ...
        if response.status_code != 204:
            logger.error(
                f"Failed to start container {container_id}. Status: {response.status_code}, Response: {response.text}"
            )
...
def stop_bot_container(container_id: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            logger.error(
                f"Error stopping container {container_id}. Status: {response.status_code}, Body: {response.text}"
            )
            response.raise_for_status()
...
async def get_running_bots_status(user_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(
            f"[Bot Status] Failed to list containers via socket API for user {user_id}: {sock_err}",
            exc_info=True,
        )
        return []  # Return empty on error listing containers
    except Exception as e:
        logger.error(
            f"[Bot Status] Unexpected error listing containers for user {user_id}: {e}",
            exc_info=True,
        )
        return []
            ...
            if meeting_id_int is not None:
                try:
                    ...
                    if meeting:
                        ...
                        logger.debug(
                            f"[Bot Status] Found DB details for meeting {meeting_id_int}: platform={platform}, native_id={native_meeting_id}"
                        )
                    else:
                        logger.warning(
                            f"[Bot Status] No meeting found in DB for ID {meeting_id_int} parsed from container '{name}'"
                        )
                except Exception as db_err:
                    logger.error(
                        f"[Bot Status] DB error fetching meeting {meeting_id_int}: {db_err}",
                        exc_info=True,
                    )
...
async def verify_container_running(container_id: str) -> bool:
    ...
    try:
        ...
        logger.error(
            f"[Verify Container] Unexpected error inspecting container {container_id}: {e}",
            exc_info=True,
        )
