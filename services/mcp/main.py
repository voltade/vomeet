...
class RequestMeetingBot(BaseModel):
    native_meeting_id: str = Field(
        ...,
        description="The unique identifier for the meeting (e.g., 'xxx-xxxx-xxx' from Google Meet URL)",
    )
    ...
    platform: str = Field(
        "google_meet",
        description="The meeting platform (e.g., 'google_meet', 'zoom'). Default is 'google_meet'.",
    )
...
async def make_request(
    method: str, url: str, api_key: str, payload: Optional[dict] = None
):
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            response = await client.request(
                method, url, headers=get_headers(api_key), json=payload
            )
